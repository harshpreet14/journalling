  ...
  useEffect(() => {
    ...
    const fetchUserEntries = async () => {
      try {
        ...
        const response = await fetch('http://localhost:3000/journal-api/v1/entries', {
          method: 'GET',
          headers: {
  ...
  return (
    <div className="flex-grow w-1100 bg-white p-6 ml-10 mr-10 rounded-2xl border-2 border-black mt-20 mb-10 flex flex-col justify-between">
      ...
      <div className="flex items-center justify-center mt-6">
        <Audio/>
        <button className="bg-[#FFC000] ml-10 hover:bg-yellow-500 text-white font-bold py-3 px-6 rounded-2xl" onClick={postEntry}>Save</button>

        <button className="bg-[#FFC000] ml-5 hover:bg-yellow-500 text-white font-bold py-3 px-6 rounded-2xl" onClick={postEntry}>Delete</button>
      </div>
    </div>
  );
};
